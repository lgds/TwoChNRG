...
print_help()
{
  ...
  echo " -d : removes ONLY rho*.bin, output*DM*NRG*.txt Rho*.dat GT_x_*.dat rho_*OmegaRhow*dat files" 
  exit 1 
}

rmoutput=0
dmnrgonly=0
while getopts "i:f: d o h" option; do
  case $option in
    i) dir0=$OPTARG ;;
    f) dirF=$OPTARG ;;
    d) dmnrgonly=1;;
...
if [ $dmnrgonly -eq 1 ]; then
  ...
  echo "rho*.bin output*DM*NRG*.txt Rho*.dat GT_x_*.dat rho_*OmegaRhow*dat"
