...
bool TwoChQS_cd_check(CNRGbasisarray *pAeigCut, int iblock1, int iblock2)
{
  ...
  double Pi=1.0;
  if (pAeigCut->NQNumbers==3){Pi=pAeigCut->GetQNumber(iblock1,2);}

  double Qj=pAeigCut->GetQNumber(iblock2,0);
  double Sj=pAeigCut->GetQNumber(iblock2,1);
  double Pj=1.0;
  if (pAeigCut->NQNumbers==3){Pj=pAeigCut->GetQNumber(iblock2,2);}
...
//   double siteqnums[]={Qtilde,Stilde,Sztilde};
  ...
  // Including the possibility of partiy/Other QNs
...
double TwoChQS_HN_MatEl(vector<double> Params,
  ...
  //  chi_Nch1         - Params[1]
  //  chi_Nch2         - Params[2]
  //  <Qm1old Sold|fN_ch1|Qold Sold> - MatArray[0] (reduced)
  //  <Qm1old Sold|fN_ch2|Qold Sold> - MatArray[1] (reduced)
  ...
  if (ist==jst){ // Diagonal terms
    ...
    for (int ich=1;ich<=2;ich++){
      OldEl[ich-1]=MatArray[ich-1].GetMatEl(stcfi,stcfj);
      int typep=typei;
      int type=typej;

      if (dEqual(fabs(OldEl[ich-1]),0.0)){
	OldEl[ich-1]=MatArray[ich-1].GetMatEl(stcfj,stcfi);
	typep=typej;
	type=typei;
      }
...
// 		 << endl;
	  ...
	  if ( (dLEqual(fabs(Sztildep),Stildep))&&
	    ...
	    //Loop in site blocks!! Necessary!
	    for (int sitestatep=pSingleSite->GetBlockLimit(siteblockp,0);
		 sitestatep<=pSingleSite->GetBlockLimit(siteblockp,1);sitestatep++){
	      for (int sitestate=pSingleSite->GetBlockLimit(siteblock,0);
		   sitestate<=pSingleSite->GetBlockLimit(siteblock,1);sitestate++){
	...
	MatEl+=chi_N[ich-1]*OldEl[ich-1]*FullMatEl;
...
//       double siteqnumsj[]={Qtildej,Stildej,Sztildej};
      ...
      // Includes the possibility of partiy/Other QNs
      int NqnsSite=pSingleSite->NQNumbers;
      double* siteqnumsi=new double [NqnsSite];
      double* siteqnumsj=new double [NqnsSite];

      for (int iqn=0;iqn<NqnsSite;iqn++){
	siteqnumsi[iqn]=pSingleSite->GetQNumber(iblssi,iqn);
	siteqnumsj[iqn]=pSingleSite->GetQNumber(iblssj,iqn);
      }
...
double TwoChQS_fN_MatEl(CNRGbasisarray* pAbasis, 
  ...
  double Si=pAbasis->GetQNumberFromSt(ist,1);
  double Pi=1.0;
  if (pAbasis->NQNumbers==3){Pi=pAbasis->GetQNumberFromSt(ist,2);}

  double Qj=pAbasis->GetQNumberFromSt(jst,0);
  double Sj=pAbasis->GetQNumberFromSt(jst,1);
  double Pj=1.0;
  if (pAbasis->NQNumbers==3){Pj=pAbasis->GetQNumberFromSt(jst,2);}
...
// 	     << endl;
	...
	for (
	  ...
	  for (
	       ...
	       sitestatej<=pSingleSite->GetBlockLimit(blockstatej,1);
	       sitestatej++){
	    auxBasis+=TwoChQS_fd_table(ich,sigma,sitestatej,sitestatei)*auxCG[0]*auxCG[1]*TwoChQS_SpSm_table(sitestatei,typei)*TwoChQS_SpSm_table(sitestatej,typej);

// 	    cout << " sitestatei = " << sitestatei
// 		 << " sitestatej = " << sitestatej
// 		 << " f_("<< sitestatei << "," << sitestatej << ") = " 
// 		 << TwoChQS_fd_table(ich,sigma,sitestatej,sitestatei) << endl
// 		 << " Connection("<< sitestatei << "," << typei << ") = " 
// 		 << TwoChQS_SpSm_table(sitestatei,typei) << endl
// 		 << " Connection("<< sitestatej << "," << typej << ") = "
// 		 << TwoChQS_SpSm_table(sitestatej,typej) << endl;
...
double TwoChQS_fNch1_MatEl(CNRGbasisarray* pAbasis, 
		       CNRGbasisarray* pSingleSite,
		       int ist, int jst){
  return( TwoChQS_fN_MatEl(pAbasis,pSingleSite,ist,jst,1) );
...
double TwoChQS_fNch2_MatEl(CNRGbasisarray* pAbasis, 
		       CNRGbasisarray* pSingleSite,
		       int ist, int jst){
  return( TwoChQS_fN_MatEl(pAbasis, pSingleSite,ist,jst,2) );
