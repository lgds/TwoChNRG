...
int TwoChQSz_SetH0Kondo(vector<double> Params,CNRGbasisarray* pSingleSite,CNRGarray* pAeig,CNRGbasisarray* pAbasisH0){
  ...
  // Construct basis for Hm1 : |up>,|dn> = |Sz=+0.5>, |Sz=-0.5>


  CNRGarray AeigHimp(1);

  AeigHimp.Nshell=-1;
  AeigHimp.NQNumbers=1;
  ...
  vector<int> CommonQNs;
  vector<int> totSpos; // empty
  
  CommonQNs.push_back(1); // tot no of common QNs
  CommonQNs.push_back(0); // pos in old
  CommonQNs.push_back(2); // pos in pSingleSite
  

  BuildBasis(CommonQNs, totSpos,&ACut,  pAbasisH0, pSingleSite,1);
